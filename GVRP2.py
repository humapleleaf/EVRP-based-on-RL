...
class GVRPDataset(Dataset):
    def __init__(self, train_size, num_nodes, t_limit, capacity, num_afs=3, data_dir=None, seed=520):
        ...
        self.dynamic = torch.ones(train_size, 4, seq_len, device=device)   # time duration, capacity, demands
        ...
        self.dynamic[:, 3, :] = self.distances[:, 0, :]
...
def train(args):
    ...
    # path = os.path.join(args.save_dir, f'gvrp{args.num_nodes}_10_final.pt')
    if args.checkpoint:
...
def update_fn(old_idx, idx, mask, dynamic, distances, dis_by_afs, capacity, velocity, cons_rate, t_limit, num_afs):
    ...
    dynamic = torch.cat((time.unsqueeze(1), fuel.unsqueeze(1), demands.unsqueeze(1),
                         distances[torch.arange(distances.size(0)), idx.squeeze(1)].unsqueeze(1)), dim=1).to(device)
    ...
    time_pd0[:, 1:num_afs+1] += 0.25
    time_pd0[:, num_afs+1:] += 0.5
    ...
    all_masked = mask.eq(0).sum(1).le(0)
    mask[all_masked, 0] = 0  # unmask the depot if all nodes are masked
    # print('old_id:', old_idx)
    # print('id:', idx)
    # print('mask:', mask)
    # match = (idx.eq(11) & old_idx.eq(22)).squeeze(1)
    # if match.any():
    #     print('----------')
    #     print(dynamic[match])
    #     print(mask[match])
    #     print('fuel_pd0:', fuel_pd0[match])
    #     print('time_pd0', time_pd0[match])
    #     # print('dis_by_afs:', dis_by_afs[match])
